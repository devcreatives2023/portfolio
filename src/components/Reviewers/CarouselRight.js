  ...
  return (
    ...
    >
      <div className="relative w-[100%] h-[100%]">
        {data.map((item, index) => {
          return (
            ...
            >
              <img
                src={item.img}
                className="xl:w-[600px] lg:w-[600px] xl:h-[500px] lg:h-[500px] w-[290px] h-[210px] object-cover border-[4px] border-yellow"
              />
              <motion.div
                className="leading-10 font-semibold text-left w-[60%] xl:text-[1.6rem] lg:text-[1.6rem] xl:mt-12 lg:mt-12 text-sm  mt-5 uppercase"
              >
                <p>{item.name} </p>
                <p>{item.text} </p>
              </motion.div>
        ...
        {/* pagination */}
        <div className="absolute invisible xl:visible lg:visible bottom-[-440px] left-[42%] translate-[50%]">
          {data.map((item, index) => {
            return (
              <div
                key={index}
